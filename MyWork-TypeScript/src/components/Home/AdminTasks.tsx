// src/components/AdminTasks.tsx
import React, { useState, useEffect } from 'react';
import { getDatabase, ref, onValue, set, remove } from 'firebase/database';
import './homeTasks.css';

// Görevler için TypeScript arayüzü
interface Task {
  key: string;
  displayKey: number;
  summary: string;
...
const AdminTasks: React.FC = () => {
  const [tasks, setTasks] = useState<Task[]>([]); // Taskleri tutmak için state

  // Bileşen yüklendiğinde taskleri Firebase'den çekmek için kullanılıyor
  useEffect(() => {
    ...
    onValue(tasksRef, (snapshot) => {
      ...
      if (data) {
        ...
        loadedTasks.sort((a, b) => a.displayKey - b.displayKey); // Taskler displayKey'e göre sıralanıyor
        setTasks(loadedTasks);
      }
    });
  }, []);

  // Tasklerin durumunu değiştiren işlev
  ...
  // Tasklerdeki metin alanlarını güncelleyen işlev
  const handleInputChange = (index: number, field: keyof Task, value: string) => {
    ...
    newTasks.sort((a, b) => a.displayKey - b.displayKey); // Taskleri displayKey'e göre sıralıyor
    setTasks(newTasks);
    updateTasksInDatabase(newTasks);
  };

  // Taskleri Firebase veritabanında güncelleyen işlev
